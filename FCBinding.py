...
# WBO_Integration Start ========================================================
# Fully isolated management - Core logic moved to WBO_Integration.py
# ==============================================================================

import sys

if not hasattr(ModernMenu, "_wbo_patch_running"):
    ModernMenu._wbo_patch_running = False

def _safe_wbo_call(method_name, self_instance, original_func, *args, **kwargs):
    """Helper macro function to safely execute external bridge patches with asynchronous decoupling and editing overrides."""
    import FreeCAD
    import FreeCADGui
    from PySide import QtCore, QtWidgets

    # CORRETTO: Rimosso il duplicato "Preferences/Mod" dalla stringa del percorso
    p_ribbon = FreeCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Ribbon")
    if not p_ribbon.GetBool("EnableWBOIntegration", True):
        if original_func:
            try: original_func(self_instance)
            except Exception: pass
        return

    if getattr(ModernMenu, "_wbo_patch_running", False):
        if original_func:
            try: original_func(self_instance)
            except Exception: pass
        return
    ...
    try:
        ...
        def deferred_wbo_apply():
            if getattr(ModernMenu, "_wbo_patch_running", False):
                return
            try:
                ModernMenu._wbo_patch_running = True

                import WBO_Integration
                WBO_Integration.apply_wbo_ribbon_patch(self_instance)

                try:
                    active_wb_name = FreeCADGui.activeWorkbench().name()
                    import WBO_Integration
                    g_dict, s_wbs, name_map = WBO_Integration.load_wbo_config()

                    if is_editing_mode:
                        target_label = name_map.get(active_wb_name, {}).get("native", active_wb_name)
                    else:
                        target_label = name_map.get(active_wb_name, {}).get("wbo", active_wb_name)

                    if target_label.endswith("Workbench") and active_wb_name in name_map:
                        target_label = target_label[:-9]

                    for tab_bar in self_instance.findChildren(QtWidgets.QTabBar):
                        for idx in range(tab_bar.count()):
                            if tab_bar.tabText(idx).strip() == target_label.strip():
                                tab_bar.setCurrentIndex(idx)
                                tab_bar.update()
                                break
                except Exception as focus_err:
                    print(f"[WBO-Focus-Hook] Failed auto-focus: {focus_err}")

            except Exception as wbo_err:
                print(f"WBO-Bridge: Error applying visual layout overlay: {wbo_err}")
            finally:
                ModernMenu._wbo_patch_running = False

        delay_ms = 10 if is_editing_mode else 0
        QtCore.QTimer.singleShot(delay_ms, deferred_wbo_apply)

    except Exception as e:
        print(f"WBO-Bridge: Error inside patched method execution {method_name}: {e}")

# ==============================================================================
# UNBREAKABLE REGISTRY SYSTEM: Uses Python sys modules to save real pointers
# ==============================================================================
if not hasattr(sys, "_wbo_original_methods_registry"):
    sys._wbo_original_methods_registry = {}

# --- 1. PATCH FOR __INIT__ ---
if "__init__" not in sys._wbo_original_methods_registry:
    sys._wbo_original_methods_registry["__init__"] = ModernMenu.__init__

    def safe_patched_init(self, *args, **kwargs):
        sys._wbo_original_methods_registry["__init__"](self, *args, **kwargs)
        try:
            import WBO_Integration
            WBO_Integration.setup_async_startup(self, ModernMenu)
        except Exception as e:
            print(f"[WBO-Bridge] Error delegating asynchronous initialization: {e}")

    ModernMenu.__init__ = safe_patched_init

# --- 2. PATCH FOR ONWBACTIVATED ---
# Cleaned signature mapping to strictly match ModernMenu.onWbActivated(self) requirements
if "onWbActivated" not in sys._wbo_original_methods_registry and hasattr(ModernMenu, "onWbActivated"):
    sys._wbo_original_methods_registry["onWbActivated"] = ModernMenu.onWbActivated
    ModernMenu.onWbActivated = lambda self, *a, **k: _safe_wbo_call(
        "onWbActivated", self, sys._wbo_original_methods_registry["onWbActivated"]
    )

# --- 3. PATCH FOR CREATEMENUS ---
if "CreateMenus" not in sys._wbo_original_methods_registry and hasattr(ModernMenu, "CreateMenus"):
    sys._wbo_original_methods_registry["CreateMenus"] = ModernMenu.CreateMenus

    def patched_create_menus(self, *args, **kwargs):
        sys._wbo_original_methods_registry["CreateMenus"](self, *args, **kwargs)
        try:
            import WBO_Integration
            WBO_Integration.inject_wbo_switch(self)
        except Exception as e:
            print(f"[WBO-Bridge] Error delegating custom switch injection: {e}")

    ModernMenu.CreateMenus = patched_create_menus
